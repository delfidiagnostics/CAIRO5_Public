...
# restrict to arm 1
finalData <- finalData[`Study Arm` == 3]
...
FigS9B <- ggplot() +
  ...
  scale_color_manual("",values=c("DELFI-TF"="#046C9A")) +
  facet_wrap(~`Subject ID`, scales = 'free_x', ncol=5) +
...
# save plot
png(filename="Figures/FigS9B.png", height=18, width=14, units="in", res=150)
FigS9B
dev.off()

cairo_pdf(file="Figures/FigS9B.pdf", height=18, width=14)
FigS9B
dev.off()
