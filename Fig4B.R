...
# clinical data
keep <- c('Subject ID', 'PFS Registration', 'PFS Status (0=alive; 1=progression or death)')
...
tableTheme <- theme_void() +
  theme(axis.ticks.y=element_blank(), plot.title = element_blank())
...
Fig4B$table <- Fig4B$table + theme(axis.text.y.left = element_text(size=40))

# save plot
png(filename="Figures/Fig4B.png", height=16, width=18, units="in", res=150)
Fig4B
dev.off()

cairo_pdf(file="Figures/Fig4B.pdf", height=16, width=18)
Fig4B
dev.off()
