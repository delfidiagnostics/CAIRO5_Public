...
# melt
finalData <- melt(finalData, id.vars=c('Subject ID', 'PFS Registration', 'PFS Status (0=alive; 1=progression or death)'), measure.vars = c('mafStatus', 'dmsStatus'))
...
# do the survival fit
fit <- survfit(Surv(`PFS Registration`, `PFS Status (0=alive; 1=progression or death)`) ~ value, data = finalData)

# calculate hazard ratio
cox <-coxph(Surv(`PFS Registration`, `PFS Status (0=alive; 1=progression or death)`) ~ value, data = finalData)
cox <- summary(cox)
...
FigS7B$table <- FigS7B$table + theme(axis.text.y.left = element_text(size=40))

# save plot
png(filename="Figures/FigS7B.png", height=16, width=18, units="in", res=150)
FigS7B
dev.off()

cairo_pdf(file="Figures/FigS7B.pdf", height=16, width=18)
FigS7B
dev.off()
